...
- **CI**: `.github/workflows/ci.yml` runs on every push/PR to `main` — a `backend` job (`pip install -r requirements.txt && pytest`, no services needed since `conftest.py`'s SQLite setup is fully self-contained) and a `frontend` job (`npm ci`, then lint/typecheck/build via the `frontend/package.json` scripts — `typecheck` runs `tsc --noEmit`, added alongside this workflow since it didn't exist as a script before). This is CI only, not CD: Railway and Vercel already auto-deploy on push to `main` via their own GitHub integration (see Deployment below), independently of this workflow's pass/fail — there's no branch protection wiring the two together, since the project's workflow has been direct commits to `main` rather than PRs.
