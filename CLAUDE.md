...
1. **Receiving / SLA queue** (flagship feature — most important)
   ...
   - **Pallet Kanban board**: each pallet carries a status — `backlog` → `staged` → `in_progress` → `completed` — shown as one global Kanban board across all active orders. Only one order's `backlog` pallets auto-promote to `staged` at a time: the highest-priority (lowest `position`) active order that still has any pallets in `backlog`/`staged` — the "current" order. Once that order is fully drained (every pallet has moved on to `in_progress`/`completed`, i.e. nothing left in `backlog` or `staged`), the next order in the queue becomes current and its `backlog` pallets auto-stage — so sorters always have a staged order to pull from instead of waiting on whichever order happens to be #1. If a pallet is dragged back into `staged` for an earlier order, that order reclaims "current" and any lookahead-staged pallets on later orders revert to `backlog` (pallets already `in_progress`/`completed` are never touched by this). When every pallet on an order reaches `completed`, the order is soft-archived (`archived_at` set) and disappears from both the SLA queue and the Kanban board. Sorters can drag any pallet card to any column at any time — the auto-staging just sets a sensible default, it doesn't lock anything. The **Backlog** column starts collapsed on load (`KanbanColumn.tsx`'s `collapsible` prop, only set for `backlog`) since it's typically the largest and least-actionable column — a "Show"/"Hide" toggle expands it, staying a valid drop target either way since the collapse only hides the rendered cards, not the droppable container.
...
- **ID input formatting**: `frontend/components/IdInput.tsx` auto-formats order/pallet IDs live as digits are typed (e.g. typing `4201` shows `PLT-0004201`), used for `order_number` (`NewOrderForm`) and each pallet row's `pallet_id`. It tracks the typed digits as their own React state rather than re-deriving them from the padded display — parsing "how many of these zeros are real vs. padding" back out of `PLT-0000042` is genuinely ambiguous, so instead every keystroke is intercepted at `onKeyDown` (digit → append, backspace/delete → drop the last raw digit, anything else printable → blocked) and the display is recomputed fresh from that state each time. Cursor is pinned to the end after every change (`useLayoutEffect`), matching the field's only real editing pattern — append/remove from the end, not edit-in-the-middle. `onChange` is kept as a fallback (some mobile virtual keyboards don't fire reliable `keydown`), and `onPaste` extracts digits from whatever was pasted. Backend validation (previous bullet) is what actually enforces the format; this component exists so a user doesn't need to know the pattern to hit it. It also takes an optional `onValueChange` callback (a `useEffect` keyed on the computed value, deliberately not on the callback's identity) so a parent can observe it without making it fully controlled.
- **NewOrderForm gating/auto-add**: pallet rows aren't individually `required` — an order just needs at least one filled, tracked via a `Record<rowKey, boolean>` fed by each row's `IdInput onValueChange`, alongside plain controlled state for `client_name`/`sla_due_date` and `order_number`'s own `onValueChange`. "Log order" is `disabled` unless client name, order number, due date, and at least one pallet are all filled — enforced in JS rather than relying on native `required` on every row, since a trailing empty row is meant to be left alone, not filled or deleted. Typing into what's currently the *last* pallet row auto-appends a fresh empty one below it (checked against `palletRows` at the moment the row's value goes from empty to non-empty, so it fires exactly once per row, not on every keystroke after). `lib/actions.ts`'s `createOrder` already filtered out empty `pallet_id` rows before this — that's what makes it safe to leave a trailing empty row and just submit.
...
- **Mobile**: no separate mobile UI — one responsive codebase, fixed via a targeted audit (real phone-viewport screenshots + measured tap targets, not just a design pass) rather than a redesign, since the base Tailwind grid/flex layouts already collapsed to single-column correctly.
  - **Touch drag-and-drop**: `OrderCard.tsx` and `KanbanCard.tsx` (the two dnd-kit draggable elements) both set `touchAction: "none"` in their inline `style`. dnd-kit only applies that automatically to its own `<DragOverlay>` component, which this app doesn't use (it transforms the dragged element directly instead) — without it, a touch drag gets consumed by the browser as a page scroll before dnd-kit's `PointerSensor` sees it move, since scroll-vs-drag gesture arbitration happens at the browser/compositor level, not in JS. Confirmed via dnd-kit's own source, not just docs.
  - **Pallet row layout**: `NewOrderForm.tsx`'s per-pallet row was a single non-wrapping flex row (ID input, rack combobox, remove button); at phone width the rack combobox got squeezed down to only show 2-3 characters of whatever rack was selected (e.g. "CA Commodity Floor" rendered as just "CA") — a real, screenshot-confirmed bug, not just tight spacing. Fixed by stacking the row (`flex-col sm:flex-row`): the pallet ID gets its own full-width line, with the rack combobox + remove button paired on the line below (`sm:w-64` on desktop where there's room for all three side by side).
  - **Tap targets**: nav links/logout (`app/(app)/layout.tsx`), the Backlog collapse toggle (`KanbanColumn.tsx`), and the remove-pallet × button (`NewOrderForm.tsx`) all measured well under the ~44px touch-target minimum (as low as 30×16) in the audit. Fixed with Tailwind's negative-margin-cancels-padding trick (`-my-3 py-3`, `-m-2 p-2`, etc.) so the clickable area grows without shifting any visible layout — verified with a before/after desktop screenshot diff, not just re-measuring.
- **Editing/deleting orders**: each `OrderCard` in the SLA queue has a "⋮" (`OrderMenu.tsx`) with Edit/Delete. Backend: `PATCH /orders/{id}` (new `OrderUpdate` schema — `client_name`/`order_number`/`sla_due_date`, same validation as create) updates order fields only, never touching `position` or pallets, so it doesn't need to call `sync_staging()` — unlike reorder/delete, editing these fields can't change which order is "current". `DELETE /orders/{id}` already existed on the backend (tested, cascades pallets) but had no frontend entry point until now.
  - `OrderCard` is itself a dnd-kit drag handle (the whole `<li>`), so `OrderMenu`'s wrapper stops `onPointerDown` propagation — without it, opening the menu would also register as the start of a card drag. While editing, `useSortable`'s `disabled` option is set from `isEditing` *and* `{...attributes, ...listeners}` are only spread onto the `<li>` when not editing, so the edit form's inputs never fight a stray drag listener.
  - Delete has an inline two-step confirm inside the same dropdown (not a native `confirm()` or a separate modal) — clicking Delete swaps the menu to a "Delete this order?" / Delete / Cancel view rather than deleting immediately, since this cascades every pallet on the order.
  - `IdInput` gained an optional `defaultValue` prop (`EditOrderForm` uses it to pre-fill `order_number`) — `NewOrderForm`'s creation flow never needed one since it always starts blank, so this was additive, not a behavior change to the existing create path.
...
- **CI**: `.github/workflows/ci.yml` runs on every push/PR to `main` — a `backend` job (`pip install -r requirements.txt && pytest`, no services needed since `conftest.py`'s SQLite setup is fully self-contained) and a `frontend` job (`npm ci`, then lint/typecheck/build via the `frontend/package.json` scripts — `typecheck` didn't exist as a script before this workflow). This is CI only, not CD: Railway and Vercel already auto-deploy on push to `main` via their own GitHub integration (see Deployment below), independently of this workflow's pass/fail — there's no branch protection wiring the two together, since the project's workflow has been direct commits to `main` rather than PRs.
  - `typecheck` is `next typegen && tsc --noEmit`, not bare `tsc --noEmit`. Next's App Router generates route-typing helpers (`LayoutProps`, etc., referenced by the auto-managed `next-env.d.ts`) into `.next/types/` as a side effect of `next dev`/`next build` — locally that directory already exists from normal dev work, so a bare `tsc --noEmit` looks fine. A CI runner starts from a clean checkout with no `.next/` at all, so the same command fails with "Cannot find name 'LayoutProps'" (caught by actually watching the first CI run fail, not assumed). `next typegen` (Next 15.5+) generates just those types without a full build, fixing it for both CI and any other from-clean-checkout run.
...
- CI added: GitHub Actions (`.github/workflows/ci.yml`) runs backend pytest and frontend lint/typecheck/build on every push/PR to `main` (see Testing above). Deploys themselves are unaffected — Railway/Vercel keep auto-deploying on push regardless of this workflow's result.
- Mobile audit + fixes done (see Tech stack above): touch drag-and-drop, the squeezed rack-location field, and undersized tap targets were all found via a real phone-viewport Playwright audit and fixed. Verified with both a re-run of the same audit (all three issues resolved, no new ones) and a desktop screenshot diff to confirm no visual regression there.
- Order editing/deleting done (see Tech stack above): a "⋮" menu on each `OrderCard` in the SLA queue opens Edit (inline form, pre-filled) or Delete (inline confirm step). Backend's `DELETE /orders/{id}` already existed and was already tested; `PATCH /orders/{id}` is new, also covered by pytest. Verified in a real browser: menu open/close, click-outside-to-close, edit save + persist, edit cancel discards changes, delete confirm/cancel, and a real delete removing the order — plus a regression check that mouse drag-and-drop reordering still works after touching `OrderCard`'s drag-handle logic.
