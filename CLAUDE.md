...
- **Rack location**: a plain `<select>` in `NewOrderForm` (`frontend/lib/rackLocations.ts`), not free text — `RACK_LOCATIONS` is currently a placeholder list matching `seed.sh`'s 4 demo racks (`CA01-RCK01`, `CA01-RCK02`, `CA02-RCK03`, `CA03-RCK01`), not the real warehouse's full rack layout, since that hasn't been provided yet. Defaults to `"CA Commodity Floor"` (a real stored string, not `null`) for the shared no-rack floor area described above — older seed data instead used a literal `null` for the same concept, so pre-existing pallets show no rack-location line on their Kanban card while anything created through the form now shows "CA Commodity Floor" explicitly. Both are valid, just a small historical inconsistency in demo data, not a bug.
